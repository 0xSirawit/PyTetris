    ...
    class Tetromino:
        ...
        def rotate(self, board, direction):
            k = -1 if direction == "rorate-left" else 1  
            rotated_grid = np.rot90(self.grid, k=k)
            new_positions = []
            size = rotated_grid.shape[0]
            
            origin_x = self.current_each_tile_pos[0][1]
            origin_y = self.current_each_tile_pos[0][0]

            size = rotated_grid.shape[0]
            for i in range(size):
                for j in range(size):
                    if rotated_grid[i, j] == 1:
                        new_x = origin_x + (j - size // 2)
                        new_y = origin_y + (i - size // 2)
    ...
    class Board:
        ...
        def rotate_tetromino(self, direaction):
            self._current_tetromino.rotate(self,direaction)
            self.render()
